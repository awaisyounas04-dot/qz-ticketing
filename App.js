...
function StatusPill({ status }) {
  ...
  return (
    <span className={`status-pill ${meta.pill}`}>
...
// ── Demo data ─────────────────────────────────────────────────────────────────
const DEMO_TICKETS = [
  { ticket_number: 'QZ-100001', device_type: 'Siemens S7-400 PLC', serial: '6ES7 412-2XK07', issue_description: 'CPU module not booting, fault LED active', priority: 'high', technician: 'Ahmad K.', estimated_cost: 4500, parts_cost: null, labour_cost: null, actual_cost: null, completion_notes: null, notes: 'Customer needs urgent turnaround', status: 'new', created_at: new Date(Date.now() - 2 * 86400000).toISOString(), updated_at: new Date().toISOString() },
  { ticket_number: 'QZ-100002', device_type: 'Allen Bradley CompactLogix', serial: '1769-L36ERMS', issue_description: 'I/O card replacement and re-configuration required', priority: 'normal', technician: 'Omar S.', estimated_cost: 1800, parts_cost: null, labour_cost: null, actual_cost: null, completion_notes: null, notes: '', status: 'diagnosed', created_at: new Date(Date.now() - 5 * 86400000).toISOString(), updated_at: new Date().toISOString() },
  { ticket_number: 'QZ-100003', device_type: 'Beckhoff CP2621-0000', serial: 'CP2621-0000', issue_description: 'HMI touch screen unresponsive, cracked panel', priority: 'normal', technician: 'Ahmad K.', estimated_cost: 2200, parts_cost: null, labour_cost: null, actual_cost: null, completion_notes: null, notes: '', status: 'awaiting_approval', created_at: new Date(Date.now() - 4 * 86400000).toISOString(), updated_at: new Date().toISOString() },
...
export default function App() {
  ...
  const changeStatus = useCallback(async (ticketNumber, newStatus, extra = {}) => {
    ...
    // Auto-compute actual_cost if not provided but parts+labour are
  ...
  const revenue = tickets.filter(t => t.status === 'completed' && t.actual_cost)
    .reduce((a, t) => a + Number(t.actual_cost), 0);
    ...
    <div className="app">
      ...
      <div className="nav">
        {TAB_CONFIG.map(t => {
          ...
          const isHot = t.key === 'intake' || t.key === 'approved';
          return (
            <div key={t.key} className={`nav-tab ${tab === t.key ? 'active' : ''}`} onClick={() => { setTab(t.key); setStatus(''); }}>
              {t.label}
              <span className={`tab-badge ${isHot && count > 0 ? 'hot' : ''}`}>{count}</span>
            </div>
          );
        })}
      </div>
              ...
              {loading ? (
                ...
                filtered.map(ticket => (
                  <tr key={ticket.ticket_number}>
                    ...
                    <td>
                      <button className="action-btn view" onClick={() => setShowDetail(ticket)}>Open</button>
                    </td>
                  </tr>
                ))
              )}
